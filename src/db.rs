...
pub fn causal_tool_outcomes(
    ...
    let base = format!(
        "SELECT tool_name,
            ...
            COALESCE(SUM(CASE WHEN applied=0 AND lines_drop>0 AND {retouch}=1 THEN 1 ELSE 0 END),0),
            ...
            COALESCE(SUM(CASE WHEN applied=1 AND lines_drop>0 AND {retouch}=1 AND COALESCE(outcome_recovered,0)=0 THEN 1 ELSE 0 END),0)
         FROM compress_decisions
